...
for file_path in file_paths:
    ...
    if df_data['시설명'].isin(tennis_value).any():
        ...
        desired_reservation_time_list = ['1', '2', '3', '4', '5', '6', '7', '8']
        ...
        for change_colums in range(4):
            ...
            for i in range (8): # desired_reservation_time_list 속의 value 값의 수
                ...
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                   condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_notpaid))
                   
                   if condition.any():
                        reserved_member = df_data['예약자'][i]
                ...
                # 라이트 수동 추가
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                    condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액'].isin(desired_money))
                ...
                #관외 라이트
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                    condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_status_list))&(abs(df_data['할인전금액']-df_data['추가금액'])==3000)
                ...
                #라이트 자동 추가   
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                    condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_status_list))&(~df_data['추가금액'].isin(desired_money))&(df_data['할인전금액'] - (df_data['할인금액'] * 5 / 4) == 3000)
                ...
                #일반 예약
                    ...
                    if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액']==0)
                ...
                #일반 관외 
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                    condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_status_list))&(~df_data['추가금액'].isin(desired_money_zero))
    ...
    # 미인증 시민 라이트
                ...
                if day_of_week_number == 6 or day_of_week_number == 5:
                    ...
                    if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액']== 0)&(df_data['할인금액']== 0 )&(df_data['결제금액'].isin(no_certifiacte_people_weekend))
                        if condition.any():
                                reserved_member = condition[condition].index[0]
                ...
                if day_of_week_number != 6 and day_of_week_number != 5:
                    ...
                    if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액']==0)&(df_data['할인금액']==0)&(df_data['결제금액'].isin(no_certifiacte_people_week))
                        if condition.any():
                            reserved_member = condition[condition].index[0]
                            combined_value = f"{reserved_member} {desired_reservation_time_list_r[i]} {notsign_text}"# 엑셀에 쓰여질 문구
                            df_sch.loc[[new_index_values[j],new_index_values[j+1]], new_column_names[change_colums]] = combined_value #엑셀에서 사용할 셀의 위치
                ...
                # 동절기 반반 시간대
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                   condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list[i])& (df_data['결제금액'].isin(half_discount))
                   if condition.any():
                    reserved_member = condition[condition].index[0]
                    combined_value = f"{reserved_member} {desired_reservation_time_list_r[i]} {sign_text}"# 엑셀에 쓰여질 문구
                    df_sch.loc[[new_index_values[j],new_index_values[j+1]], new_column_names[change_colums]] = combined_value #엑셀에서 사용할 셀의 위치
            ...
            for k in range(7):
                    #현장결제
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                   condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_notpaid))
                   ...
                   if condition.any():
                    ...
                    #라이트 수동추가 
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액'].isin(desired_money))
                    ...
                    #관외 라이트 추가
                if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_status_list))&(abs(df_data['할인전금액']-df_data['추가금액'])==6000)
                        if condition.any():
                            # 조건을 만족하면 해당 행의 인덱스인 '예약회원'을 출력
                            reserved_member = condition[condition].index[0]
                    ...
                    #일반 라이트 자동추가
                if(df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_status_list))&(~df_data['추가금액'].isin(desired_money))&(df_data['할인전금액'] - (df_data['할인금액'] * 5 /4 ) == 6000)
                      ...
                      # 일반 이용     
                if(df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액']==0)    
                    ...
                    #관외 일반이용
                if(df_data['시설명'] == desired_facility_list[change_colums]).any():
                        condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_status_list))&(~df_data['추가금액'].isin(desired_money_zero))    
                ...
                if day_of_week_number == 6 or day_of_week_number == 5:
                        ...
                        if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                            condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액']==0)&(df_data['할인금액']==0)&(df_data['결제금액'].isin(no_certifiacte_people_weekend))
                ...
                if day_of_week_number != 6 and day_of_week_number != 5:
                        ...
                        if (df_data['시설명'] == desired_facility_list[change_colums]).any():
                            condition = (df_data['시설명'] == desired_facility_list[change_colums]) & (df_data['대관회차'] == desired_reservation_time_list_4[k])& (df_data['결제상태'].isin(desired_reservation_status_list))&(df_data['추가금액']==0)&(df_data['할인금액']==0)&(df_data['결제금액'].isin(no_certifiacte_people_week))
                            if condition.any():
